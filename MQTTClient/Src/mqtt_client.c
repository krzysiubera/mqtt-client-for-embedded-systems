...
void MQTTClient_connect(struct mqtt_client_t* mqtt_client)
{
	...
	char packet[sizeof(fixed_header) + sizeof(variable_header) + len_client_id];
	...
	tcp_write(mqtt_client->tcp_connection_raw.pcb, (void*) packet, sizeof(packet), 1);
	tcp_output(mqtt_client->tcp_connection_raw.pcb);
}
